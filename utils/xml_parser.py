...
base = '/home/annus/Desktop/logical_layout_analysis/new bigger dataset/'
    ...
    try:
        ...
        for label_name, polygon in polygons:
            ...
            if label_name in all_labels.keys():
                ...
                print('found new label: {}'.format(label_name))
...
def process_one(name):
    ...
    """
    img = Image.open(os.path.join(source, name+'.jpg')).convert('RGB')
    # we need this for resizing
    w, h = img.size
    new_dim = 1024
    img = img.resize((new_dim, new_dim), Image.ANTIALIAS)
    img_copy = img.copy().convert('1', dither=Image.NONE)
    # save our example
    draw = ImageDraw.Draw(img)
    ...
    # img_copy.save(os.path.join(examples, '{}.jpg'.format(name))) # save #1. image as npy array
...
def check_label_generated(name):
    ...
    """
    img = Image.open(os.path.join(examples, name+'.jpg')).convert('1', dither=Image.NONE)
